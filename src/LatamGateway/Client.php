...
class Client
{
     ...
     * @param string $typeResponse expected response type of the endpoint
     * @param string $endpointPath path to method endpoint before expanding parameters
     *
     * @throws Client\Exception on a non 2xx response
     * @return mixed
     */
    public function call($resourcePath, $method, $queryParams, $postData, $headerParams = null, $typeResponse = true, $endpointPath = null)
    {
        $headers = [];
        $authHeader = [];
        $defaultHeader = $this->config->getHeaders();
        if (null != $this->config->getAuthentication()) {
            $authHeader = $this->config->getAuthentication();
        }
        ...
        if (class_exists($typeResponse)) {
            $data = new $typeResponse($data);
        }
